...
%place themes under myDbInputPath folder
theme = 'green_theme'; % choose the theme  - romance_theme/crazy_theme/yellow_theme/green_theme
...
if(length(albumName)~=0)
    %album art copied
    ImageGoogle(myDbInputPath,char(albumName));
    ImageGoogle(myDbInputPath,char(artistName));
...
for cnt = 1 : length(timings)
   k=1;
    ...
    for i=1  : length(strings)
        ...
        [random,word] = imageforLyricNext(myDbInputPath,myDbFinalPath,char(strings(i)),imHistAlbumArt);
        display(word);
        if(random==0)
